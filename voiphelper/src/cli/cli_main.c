...
LOCAL char **cmdCompletion(const char *pText, int start, int end)
{
	...
	else
	{
		...
		/*l_pCmdMapSearched or l_pArgsSearched to the correct list and then search matchingcorrect list */
		...
		if ((searchResult = setSearchedList(0, len)) == 0)
		{
...
LOCAL void printCmdDocs(CMD_MAP_T *pCmd)
{
	printf(CLRerr"ERROR." CLRn"Invaild arguments.\nUsage : %s\n", pCmd->pDocs);

	CMD_ARG_T *pArg = pCmd->pSupportedArgs;
...
GLOBAL int main(int argc, char **argv)
{
    ...
    voiph_initConfig(".vch_config");

    initReadline();

    showWelcome();

    pthread_create(&tid, NULL, handleClientConn, NULL);
